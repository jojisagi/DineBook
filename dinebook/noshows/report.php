<?php
// report.php — No-Shows: List All
require_once __DIR__ . '/../auth/guard.php';
require_once __DIR__ . '/../config.php';
try { $records = iterator_to_array($noshows->find([], ['sort' => ['reported_at' => -1]])); } catch (Exception $e) { $records = []; $dbError = $e->getMessage(); }
?>
<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>DineBook — No-Show Reports</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet"><link rel="stylesheet" href="/dinebook/css/style.css"></head>
<body>
    ...
    <div class="container mt-4">
        <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="../index.php">Home</a></li><li class="breadcrumb-item active">No-Show Reports</li></ol></nav>
        <h2 class="page-header">All No-Show Reports</h2>
        ...
        <?php elseif (count($records) === 0): ?><div class="alert alert-warning">No no-show reports registered.</div>
        <?php else: ?>
        <p>Total: <strong><?php echo count($records); ?></strong></p>
        <div class="table-responsive"><table class="table table-striped table-hover table-bordered">
            <thead><tr><th>Guest</th><th>Email</th><th>Date</th><th>Party</th><th>Zone</th><th>Reason</th><th>Reminded</th><th>Impact</th><th>Follow-up</th><th>Reported By</th></tr></thead>
            ...
            <tr>
                ...
                <td><?php echo htmlspecialchars($r['reason_category']??''); ?></td>
                <td><?php echo htmlspecialchars($r['was_reminded']??'unknown'); ?></td>
                <td><span class="badge bg-<?php echo ($r['impact_assessment']??'low')==='high'?'danger':(($r['impact_assessment']??'low')==='medium'?'warning':'secondary'); ?>"><?php echo htmlspecialchars($r['impact_assessment']??'low'); ?></span></td>
                <td><?php echo htmlspecialchars($r['follow_up_action']??'none'); ?></td>
