<?php
// delete_process.php — No-Shows: Execute delete
require_once __DIR__ . '/../auth/guard.php';
require_once __DIR__ . '/../config.php';
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || empty($_POST['id'])) { header('Location: delete.php'); exit; }
try { $oid = new MongoDB\BSON\ObjectId($_POST['id']); $result = $noshows->deleteOne(['_id' => $oid]); $deleted = $result->getDeletedCount() === 1; } catch (Exception $e) { $deleted = false; $dbError = $e->getMessage(); }
?>
<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>DineBook — No-Show Deleted</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet"><link rel="stylesheet" href="/dinebook/css/style.css"></head>
<body>
    ...
    <div class="container mt-4">
        <h2 class="page-header">Delete No-Show</h2>
        <?php if (isset($dbError)): ?><div class="alert alert-danger"><?php echo htmlspecialchars($dbError); ?></div>
        <?php elseif ($deleted): ?><div class="alert alert-success success-msg">No-show report deleted successfully.</div>
        <?php else: ?><div class="alert alert-danger">Record not found or already deleted.</div><?php endif; ?>
