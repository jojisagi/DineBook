<?php
// create_process.php — Guests: Insert
...
$required = ['full_name', 'email', 'phone', 'loyalty_tier', 'contact_preference'];
$errors = [];
...
<head>
    ...
    <title>DineBook — Guest Created</title>
...
<body>
    ...
    <div class="container mt-4">
        <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="../index.php">Home</a></li><li class="breadcrumb-item"><a href="report.php">Guests</a></li><li class="breadcrumb-item active">Create Result</li></ol></nav>
        <h2 class="page-header">Create Guest</h2>
...
<?php else:
    try {
        $document = [
            ...
            'preferred_zone'       => $_POST['preferred_zone'] ?? '',
            ...
            'marketing_opt_in'     => (bool)isset($_POST['marketing_opt_in']),
            'notes'                => $_POST['notes'] ?? '',
            'visit_count'          => (int)($_POST['visit_count'] ?? 0),
            'last_visit'           => $_POST['last_visit'] ?? '',
            'registered_at'        => new MongoDB\BSON\UTCDateTime()
        ];
        $result = $guests->insertOne($document);
?>
        <div class="alert alert-success success-msg">Guest <strong><?php echo htmlspecialchars($document['full_name']); ?></strong> created successfully. (ID: <?php echo $result->getInsertedId(); ?>)</div>
        <a href="create.html" class="btn btn-primary">New Guest</a>
