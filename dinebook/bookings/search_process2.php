<?php
// search_process2.php — Bookings: Search by selectable field result
...
$filter = [];
switch ($field) {
    case 'assigned_by':     $filter = ['assigned_by' => $value]; break;
    case 'booking_status':  $filter = ['booking_status' => $value]; break;
    case 'payment_status':  $filter = ['payment_status' => $value]; break;
    case 'booking_date':    $filter = ['booking_date' => $value]; break;
    default:                $filter = ['assigned_by' => $value]; break;
}
try { $records = iterator_to_array($bookings->find($filter)); } catch (Exception $e) { $records = []; $dbError = $e->getMessage(); }
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="container mt-4">
        ...
        <?php elseif (count($records) === 0): ?><div class="alert alert-warning">No bookings found.</div>
        <?php else: ?>
